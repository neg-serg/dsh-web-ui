...
import { sshCommandDefinitions } from './commands.ts'
...
export const inject = ['webServer', 'tools', 'systemPrompt', 'commands']
...
export function apply(ctx: Context, config?: Config): void {
  ...
  // The engine as an injectable service: other plugins (command packs,
  // future surfaces) drive the same connection pool as the panel and tools.
  ctx.provide('ssh', engine)
  ...
  // Slash commands (/ssh, /ssh-hosts, /ssh-tunnel, /ssh-cluster).
  ...
  const sync = (): void => {
    ...
    disposeCommands = ctx.effect(
      () => {
        const disposers = sshCommandDefinitions(engine).map(definition => ctx.commands.register(definition))
        return () => { for (const dispose of disposers) dispose() }
      },
      'dsh-ssh: commands',
    )
